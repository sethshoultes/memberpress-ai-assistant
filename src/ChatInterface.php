...
class ChatInterface {
    ...
    private function renderChatContainerHTML() {
        ...
        <button id="mpai-chat-toggle" class="mpai-chat-toggle mpai-chat-toggle-<?php echo esc_attr(str_replace('_', '-', $chat_position)); ?>" data-position="<?php echo esc_attr($chat_position); ?>" aria-label="<?php esc_attr_e('Toggle chat', 'memberpress-copilot'); ?>">
    ...
    public function checkChatPermissions($request) {
        ...
        if (function_exists('current_user_can') && !current_user_can('read')) {
            return new \WP_Error(
                ...
                __('You do not have permission to use the chat.', 'memberpress-copilot'),
                ['status' => 403]
            );
        }
